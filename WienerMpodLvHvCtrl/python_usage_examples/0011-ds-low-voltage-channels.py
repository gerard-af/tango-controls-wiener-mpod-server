...
def lv_channel_test(module_slot_index, channel_index):
    print 'testing LV channel {0} in module {1}'.format(channel_index, module_slot_index)
    val = 1.0
...
for module_slot_index in occupied_slots:
    if not test_device.command_inout('ModuleGetIsHvModule', module_slot_index):
        ...
        for channel_index in range(nb_channels_per_module):
            lv_channel_test(module_slot_index, channel_index)
